...
class QMI8658(object):
    def __init__(self,address=0X6B):
        ...
        self._bus = I2C(0, scl=Pin(I2C_SDL),sda=Pin(I2C_SDA),freq=400000)
        bRet=self.WhoAmI()
        if bRet :
            self.Read_Revision()
        else    :
    ...
    def Read_gyroXYZ(self):
        ...
        for i in range(3):
            ...
            gyroxyz[i+3]=raw_xyz[i+3]*1.0/gyro_lsb_div
        return gyroxyz
    
    def Read_acclXYZ(self):
        accxyz=[0,0,0,0,0,0]
        raw_xyz=self.Read_Raw_XYZ()  
        #QMI8658AccRange_8g
        acc_lsb_div=(1<<12)
        ...
        gyro_lsb_div = 64
        for i in range(3):
            accxyz[i]=raw_xyz[i]/acc_lsb_div#(acc_lsb_div/1000.0)
